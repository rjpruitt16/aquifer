...
For a regional deployment that does not need a separate control plane, combine static cluster routing with Valkey remote idempotency: any node can receive the request, consistent hashing keeps a user's normal traffic on one owner, and `AQUIFER_DRAIN_SINK=valkey` publishes completed/failed idempotency records under the generic `aqueduct:idempotency:` prefix so another node can reject duplicates before dispatch. See [API.md](API.md#remote-idempotency) for the exact key contract.
